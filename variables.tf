...
variable "internet_to_app_ports" {
  description = "Map of ports the internet can open on app servers in app subnets, on top of the replies to the requests they make themselves. Each entry must specify from_port. Optional: to_port (defaults to from_port), protocol ('tcp' or 'udp', defaults to 'tcp' — other protocols are rejected, see below). Network ACLs are stateless, so each entry also opens the ephemeral range 1024-65535 in egress for the same protocol, which is where the reply goes. Applied to both 0.0.0.0/0 and ::/0, and only where the module provisions an internet path: var.internet_access_allowed = true, or interface endpoints required by var.vpc_endpoints_services while var.vpc_endpoints_allowed = false. Like the module's own internet rules, a narrower source belongs to the security group in front, and a network ACL wider than that security group grants nothing. Traffic only actually arrives over IPv4 where the app subnets are public (var.nat_gateways_allowed = false) and the workload takes a public address; over IPv6 the app subnets egress through an egress-only internet gateway, which admits no externally initiated connection. Cannot be used with external subnets (subnets_ids). Only 'tcp' and 'udp' are accepted: AWS ignores from_port/to_port when protocol is '-1' (all), turning a single-port entry into an allow-everything rule from 0.0.0.0/0 and ::/0, and 'icmp'/'icmpv6' give from_port/to_port a type/code meaning this module's port-range and ephemeral-reply logic doesn't handle. Security Hub: EC2.21 (Network ACLs should not allow ingress from 0.0.0.0/0 to port 22 or port 3389) — default (empty) = pass; adding an entry with from_port/to_port covering 22 or 3389 fails it."
  type = map(object({
    from_port = number
    to_port   = optional(number)
    protocol  = optional(string, "tcp")
  ...
  validation {
    condition = alltrue([
      for entry in values(var.internet_to_app_ports) : contains(["tcp", "udp"], entry.protocol)
    ])
    error_message = "internet_to_app_ports entries must have protocol = \"tcp\" or \"udp\". AWS ignores from_port/to_port when protocol is \"-1\"/\"all\", so that value turns a single-port entry into an allow-everything rule; \"icmp\"/\"icmpv6\" give from_port/to_port a type/code meaning this module doesn't support."
  }
}

variable "app_to_internet_ports" {
  description = "Map of ports app servers in app subnets can open on the internet, on top of the TCP 443 already opened. Each entry must specify from_port. Optional: to_port (defaults to from_port), protocol ('tcp' or 'udp', defaults to 'tcp' — other protocols are rejected, see below). Network ACLs are stateless, so each entry also opens the ephemeral range 1024-65535 in ingress for the same protocol, which is where the reply comes back; a 'tcp' entry adds nothing there, since that range is already open in ingress for TCP, but a 'udp' one genuinely opens it for UDP, because the module cannot know which local ports the workload binds and so opens the whole range. Applied to both 0.0.0.0/0 and ::/0, and only where the module provisions an internet path: var.internet_access_allowed = true, or interface endpoints required by var.vpc_endpoints_services while var.vpc_endpoints_allowed = false. Like the module's own internet rules, a narrower destination belongs to the security group in front, and a network ACL wider than that security group grants nothing. Cannot be used with external subnets (subnets_ids). Only 'tcp' and 'udp' are accepted: AWS ignores from_port/to_port when protocol is '-1' (all), turning a single-port entry into an allow-everything rule from 0.0.0.0/0 and ::/0 that the module's auto-generated ephemeral reply rule would inherit too, and 'icmp'/'icmpv6' give from_port/to_port a type/code meaning this module's port-range and ephemeral-reply logic doesn't handle. In the default architecture (var.nat_gateways_allowed = true), only 'tcp' entries on port 443 are known to reach the internet: the network ACL on the netdev subnet the NAT gateway sits behind only carries TCP 443 outbound and its TCP ephemeral reply inbound, so any other protocol or port opened here is dropped one hop later; set var.nat_gateways_allowed = false (public app subnets) to actually use a different protocol or port over IPv4. IPv6 egress bypasses the netdev subnet (an egress-only internet gateway is attached directly to the app route table) and is unaffected. Security Hub: EC2.21 (Network ACLs should not allow ingress from 0.0.0.0/0 to port 22 or port 3389) — default (empty) = pass; the only ingress an entry adds is that ephemeral range in the entry's own protocol, so a 'tcp' entry adds nothing the module doesn't already open, and the egress rule still widens what the app tier may reach, so list only the ports the workload calls out on."
  type = map(object({
    from_port = number
    to_port   = optional(number)
    protocol  = optional(string, "tcp")
  ...
  validation {
    condition = alltrue([
      for entry in values(var.app_to_internet_ports) : contains(["tcp", "udp"], entry.protocol)
    ])
    error_message = "app_to_internet_ports entries must have protocol = \"tcp\" or \"udp\". AWS ignores from_port/to_port when protocol is \"-1\"/\"all\", so that value turns a single-port entry into an allow-everything rule; \"icmp\"/\"icmpv6\" give from_port/to_port a type/code meaning this module doesn't support."
  }
}
