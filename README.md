...
Each entry becomes two rules per IP family: the requested range in its own direction, and the ephemeral range `1024-65535` in the other one, where the reply lands. Entries sharing a protocol share that reply rule, which is written once. Both variables only apply where the module provisions an internet path — `internet_access_allowed = true`, or interface endpoints required while `vpc_endpoints_allowed = false` — and neither can be used with external subnets (`subnets_ids`), whose network ACLs this module doesn't manage. Inbound only reaches a workload where the app subnets are public (no NAT gateway) and the workload has a public address.
