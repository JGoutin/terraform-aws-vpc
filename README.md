...
- ✅ **App Internet Flows** - Extra protocols and ports through the app network ACL (e.g. UDP media)
...
## Application Subnet Internet Flows

The app subnets' network ACL opens TCP `443` outbound and the replies to it inbound. Any other flow has to be declared: a network ACL is stateless and evaluated before any security group, so what it doesn't carry is dropped whatever the security groups allow.

```hcl
internet_access_allowed = true
nat_gateways_allowed    = false # app subnets are public, the workload takes a public IP

# UDP media the internet sends to the workload, and the STUN query it sends out
internet_to_app_ports = {
  "webrtc-media" = { from_port = 30000, to_port = 32767, protocol = "udp" }
}
app_to_internet_ports = {
  "stun" = { from_port = 3478, protocol = "udp" }
}
```

Each entry becomes the requested range in its own direction, plus the ephemeral range `1024-65535` in the other one, where the reply lands — except a `tcp` entry in `app_to_internet_ports`, whose reply rule is the same ingress rule the app NACL already carries by default, so it adds nothing further. Entries sharing a protocol share their reply rule, written once. Only `tcp` and `udp` are accepted: AWS ignores `from_port`/`to_port` when the protocol is `-1` (all), and `icmp`/`icmpv6` give those fields a type/code meaning this module doesn't handle. Both variables only apply where the module provisions an internet path — `internet_access_allowed = true`, or interface endpoints required while `vpc_endpoints_allowed = false` — a plan fails otherwise, rather than silently dropping every entry. Neither can be used with external subnets (`subnets_ids`), whose network ACLs this module doesn't manage. Inbound only reaches a workload where the app subnets are public (no NAT gateway) and the workload has a public address.

Over IPv6 the app subnets always egress through an egress-only internet gateway, which lets no connection in from outside. An IPv6 entry in `internet_to_app_ports` therefore carries the return traffic of a flow the workload opened from that port range itself; it cannot admit a caller-initiated IPv6 flow.

Neither takes a source or destination CIDR — rules are written for `0.0.0.0/0` and `::/0`. The caller restriction belongs to the security group in front, and a network ACL wider than a security group grants nothing. The module's own app security group only egresses TCP `443`, so give the workload an additional security group carrying the matching rules.

`app_to_internet_ports` only reaches the internet as configured when the app subnets are public (`nat_gateways_allowed = false`). In the default architecture (`nat_gateways_allowed = true`), egress transits a second network ACL on the netdev subnet the NAT gateway sits behind, which is hard-coded to TCP `443` outbound and its TCP ephemeral reply inbound: a `udp` entry, or a `tcp` entry on any port but `443`, passes the app NACL and is then dropped by the netdev one. IPv6 egress bypasses the netdev subnet (an egress-only internet gateway is attached directly to the app route table) and isn't affected.
...
| EC2.21 (CIS 5.1/5.2) | 🟡 Medium | Network ACLs should not allow ingress from 0.0.0.0/0 to remote administration ports (22, 3389) | ❌ Fail (accepted) | See [Why EC2.21 fails](#why-ec221-fails) below. Adding a `public_ingress_ports` / `public_to_app_ports` / `internet_to_app_ports` entry covering 22 or 3389 would fail it for a second, real reason — the defaults (443 / 8000 / none) do not. `app_to_internet_ports` only ever adds ingress on the ephemeral range `1024-65535`, already open here for TCP; a `tcp` entry opens nothing new in ingress, a `udp` one opens that range for UDP. |
...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_app_to_internet_ports"></a> [app\_to\_internet\_ports](#input\_app\_to\_internet\_ports) | Map of ports app servers in app subnets can open on the internet, on top of the TCP 443 already opened. Each entry must specify from\_port. Optional: to\_port (defaults to from\_port), protocol ('tcp' or 'udp', defaults to 'tcp' — other protocols are rejected, see below). Network ACLs are stateless, so each entry also opens the ephemeral range 1024-65535 in ingress for the same protocol, which is where the reply comes back; a 'tcp' entry adds nothing there, since that range is already open in ingress for TCP, but a 'udp' one genuinely opens it for UDP, because the module cannot know which local ports the workload binds and so opens the whole range. Applied to both 0.0.0.0/0 and ::/0, and only where the module provisions an internet path: var.internet\_access\_allowed = true, or interface endpoints required by var.vpc\_endpoints\_services while var.vpc\_endpoints\_allowed = false. Like the module's own internet rules, a narrower destination belongs to the security group in front, and a network ACL wider than that security group grants nothing. Cannot be used with external subnets (subnets\_ids). Only 'tcp' and 'udp' are accepted: AWS ignores from\_port/to\_port when protocol is '-1' (all), turning a single-port entry into an allow-everything rule from 0.0.0.0/0 and ::/0 that the module's auto-generated ephemeral reply rule would inherit too, and 'icmp'/'icmpv6' give from\_port/to\_port a type/code meaning this module's port-range and ephemeral-reply logic doesn't handle. In the default architecture (var.nat\_gateways\_allowed = true), only 'tcp' entries on port 443 are known to reach the internet: the network ACL on the netdev subnet the NAT gateway sits behind only carries TCP 443 outbound and its TCP ephemeral reply inbound, so any other protocol or port opened here is dropped one hop later; set var.nat\_gateways\_allowed = false (public app subnets) to actually use a different protocol or port over IPv4. IPv6 egress bypasses the netdev subnet (an egress-only internet gateway is attached directly to the app route table) and is unaffected. Security Hub: EC2.21 (Network ACLs should not allow ingress from 0.0.0.0/0 to port 22 or port 3389) — default (empty) = pass; the only ingress an entry adds is that ephemeral range in the entry's own protocol, so a 'tcp' entry adds nothing the module doesn't already open, and the egress rule still widens what the app tier may reach, so list only the ports the workload calls out on. | <pre>map(object({<br/>    from_port = number<br/>    to_port   = optional(number)<br/>    protocol  = optional(string, "tcp")<br/>  }))</pre> | `{}` | no |
...
| <a name="input_internet_to_app_ports"></a> [internet\_to\_app\_ports](#input\_internet\_to\_app\_ports) | Map of ports the internet can open on app servers in app subnets, on top of the replies to the requests they make themselves. Each entry must specify from\_port. Optional: to\_port (defaults to from\_port), protocol ('tcp' or 'udp', defaults to 'tcp' — other protocols are rejected, see below). Network ACLs are stateless, so each entry also opens the ephemeral range 1024-65535 in egress for the same protocol, which is where the reply goes. Applied to both 0.0.0.0/0 and ::/0, and only where the module provisions an internet path: var.internet\_access\_allowed = true, or interface endpoints required by var.vpc\_endpoints\_services while var.vpc\_endpoints\_allowed = false. Like the module's own internet rules, a narrower source belongs to the security group in front, and a network ACL wider than that security group grants nothing. Traffic only actually arrives over IPv4 where the app subnets are public (var.nat\_gateways\_allowed = false) and the workload takes a public address; over IPv6 the app subnets egress through an egress-only internet gateway, which admits no externally initiated connection. Cannot be used with external subnets (subnets\_ids). Only 'tcp' and 'udp' are accepted: AWS ignores from\_port/to\_port when protocol is '-1' (all), turning a single-port entry into an allow-everything rule from 0.0.0.0/0 and ::/0, and 'icmp'/'icmpv6' give from\_port/to\_port a type/code meaning this module's port-range and ephemeral-reply logic doesn't handle. Security Hub: EC2.21 (Network ACLs should not allow ingress from 0.0.0.0/0 to port 22 or port 3389) — default (empty) = pass; adding an entry with from\_port/to\_port covering 22 or 3389 fails it. | <pre>map(object({<br/>    from_port = number<br/>    to_port   = optional(number)<br/>    protocol  = optional(string, "tcp")<br/>  }))</pre> | `{}` | no |
